...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers, gaussian=False):
        ...
        # MVP: diagonal-Gaussian posterior head. When gaussian=True, `out` outputs mu
        # (softmax removed) and a parallel head emits logvar. Retrieved as self.last_logvar.
        if gaussian:
    ...
    def forward(self, input, hidden):
        ...
        if self.gaussian:
            mu = self.out(h)
            self.last_logvar = self.logvar_head(h).clamp(-10.0, 2.0)
            return output, new_hidden, mu
        new_hidden_out = f.softmax(self.out(h), dim=-1)
