...
def resolve_model_paths(checkpoint_paths, load_step, logger):
    model_paths = []
    for checkpoint_path in checkpoint_paths:
        if not os.path.isdir(checkpoint_path):
            logger.console_logger.info("Checkpoint directory {} doesn't exist".format(checkpoint_path))
            return None
        model_paths.append(get_model_path(checkpoint_path, load_step))
    return model_paths


def resolve_belief_model_paths(args, logger):
    belief_checkpoint_paths = getattr(args, "belief_checkpoint_paths", [""])
    if len(belief_checkpoint_paths) == 0 or belief_checkpoint_paths[0] == "":
        belief_checkpoint_paths = args.checkpoint_paths

    if len(belief_checkpoint_paths) == 0 or belief_checkpoint_paths[0] == "":
        return None

    return resolve_model_paths(belief_checkpoint_paths, args.load_step, logger)
...
def run_sequential(args, logger):
    ...
    if args.checkpoint_paths[0] != "":
        model_paths = resolve_model_paths(args.checkpoint_paths, args.load_step, logger)
        if model_paths is None:
            return
        logger.console_logger.info("Loading models from {}".format(model_paths))
        learner.load_models(model_paths)

    # Load pretrained belief models, optionally from a separate checkpoint bundle.
    belief_model_paths = resolve_belief_model_paths(args, logger)
    if belief_model_paths is not None:
        logger.console_logger.info("Loading belief models from {}".format(belief_model_paths))

        if args.Behavior_enable:
            behavior_learner.load_models(belief_model_paths)
            if getattr(args, "belief_frozen", False):
                behavior_learner.freeze()
                logger.console_logger.info("Behavior belief module frozen after loading.")

        if args.GAT_enable:
            prediction_learner.load_models(belief_model_paths)
            if getattr(args, "belief_frozen", False):
                prediction_learner.freeze()
                logger.console_logger.info("Instant belief module frozen after loading.")
    ...
    while runner.t_env <= args.t_max:
        ...
        if args.Behavior_enable:
            ...
            elif runner.t_env > args.Behavior_warmup:
        ...
        if args.GAT_enable:
            ...
            elif runner.t_env > args.GAT_warmup:
...
def log_print(args, episode, t_env, episode_win_rate, episode_reward, episode_len,
              behavior_loss=None, stability_loss=None, total_loss=None, prediction_loss=None):

    def safe_sum(x):
        return sum(x) if x is not None else 0
    ...
    if args.env == "highway":
        ...
        if args.Behavior_enable:
            print("Behavior Loss", safe_sum(behavior_loss),
                  " | Stability Loss", safe_sum(stability_loss),
                  " | Behavior Total Loss", safe_sum(total_loss))
        if args.GAT_enable:
            print("Prediction Loss", safe_sum(prediction_loss))
        print("--------------------------------")
