...
import highway_env  # author's fork installed from github, provides hetero envs
import gym
...
def run_sequential(args, logger):
    ...
    if args.checkpoint_paths[0] != "":
        ...
        # Resume runner timestep from the loaded checkpoint
        loaded_step = int(os.path.basename(model_paths[0]))
        runner.t_env = loaded_step

    if args.evaluate:
        logger.console_logger.info("Evaluation mode: running {} test iterations (batch_size_run={})".format(
            args.test_nepisode, args.batch_size_run))
        evaluate_sequential(args, runner, learner)
        logger.console_logger.info("Evaluation finished.")
        return
